...
def login_sisgat(url_base, username, password):
    ...
    Retorna a sessão e uma mensagem de sucesso/erro.
    """
    ...
    try:
        ...
        if csrf_token:
            st.info(f"CSRF Token encontrado.")
        else:
            st.warning("CSRF Token não encontrado na página de login. Prosseguindo sem ele.")
        ...
        if "login" not in login_response.url.lower() and "Acesso não autorizado" not in login_response.text:
            ...
            st.error("Falha no login. Credenciais inválidas ou página de login inesperada.")
...
def obter_boletos_solicitados(session, url_base="https://sisgat.cbm.am.gov.br"):
    ...
    e extrai os dados da tabela, que representam os processos disponíveis.
    """
    url_boletos = f"{url_base}/boletos-solicitados"
    st.info(f"Acessando a página de boletos solicitados em: {url_boletos}")
    ...
    try:
        ...
        if 'text/html' not in response.headers.get('Content-Type', ''):
            st.error("A resposta não é HTML. Pode ser um erro ou conteúdo inesperado.")
            return []
        ...
        if not table:
            st.warning("Tabela de boletos/processos não encontrada na página. Verifique os seletores.")
            return []
        ...
        st.error(f"Erro na requisição para obter boletos solicitados: {e}")
        return []
    except Exception as e:
...
def obter_detalhes_boleto_sisgat(session, boleto_id, url_base="https://sisgat.cbm.am.gov.br"):
    ...
    e extrai os detalhes necessários, usando uma sessão autenticada.
    """
    ...
    try:
        ...
        detalhes['boleto_anexado'] = extract_detail('Boleto')
        ...
        detalhes_limpos = {k: v for k, v in detalhes.items() if v is not None and v.strip() != ''}

        if not detalhes_limpos:
            st.warning("Nenhum detalhe significativo encontrado na página de visualização do boleto.")
            return None

        return detalhes_limpos

    except requests.exceptions.RequestException as e:
        st.error(f"Erro na requisição para obter detalhes do boleto {boleto_id}: {e}")
        return None
    except Exception as e:
        st.error(f"Ocorreu um erro ao parsear os detalhes do boleto {boleto_id}: {e}")
        return None
...
# --- Barra Lateral para Login e Seleção de Processos ---
with st.sidebar:
    ...
    if 'processos_listados' in st.session_state and st.session_state['processos_listados']:
        ...
        # Encontrar o objeto do processo selecionado
        selected_process = None
        if selected_process_display != "Selecione um processo...":
            # Extrair o Nº do Processo da string selecionada para encontrar o objeto original
...
# --- Área Principal para Exibição dos Detalhes ---
...
if 'selected_process' in st.session_state and st.session_state['selected_process']:
    ...
    if processo_id and 'session' in st.session_state and st.session_state['session']:
        ...
        if detalhes_completos:
            ...
            st.warning(f"Não foi possível carregar os detalhes completos para o Processo/Boleto ID: {processo_id}.")
    else:
